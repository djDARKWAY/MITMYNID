const validateUsersEditForm = (values : any) => {
    ...
    /* else if(values.person_name && values.person_name.trim().length!==0 && !/^\S*[A-Za-zÀ-ÿ ._-]{2,}$/.test(values.person_name)){
        //@ts-ignore
        errors.person_name = 'resources.utilizadores.field_validation.name'
    } */

    if (!values.username || (values.username && values.username.trim().length===0)) {
        //inves de passar mensagens podeomos passar identificadores para o i8onprovider
        //@ts-ignore
...
export default validateUsersEditForm;