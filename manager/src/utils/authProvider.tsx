...
export const authProvider = {
  ...
  login: async ({ username, password }: any) => {
    ...
    try {
      ...
      if (!decodedToken || (decodedToken && !Array.isArray(decodedToken.roles))) return Promise.reject();

      const roles = decodedToken.roles;

      if(!roles.includes('ADMIN') || roles.includes('ARMAZEM')) return Promise.reject();
